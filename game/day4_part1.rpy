...
label day4_parents_part0_choice_for_aliya:
    ...
    me "За то короткое время, что я её знаю я успел убедиться в том," with dissolve

    me "что она вопреки Вашему о ней мнению серьёзно обдумывала данное решение." with dissolve
    ...
    me "Нет, Вы ни разу и не подумали, о таком." with dissolve

    me "Знаете, мне вот даже интересно насколько Вы знаете Вашу же дочку? Её мечты, желание, чего она бы хотела?" with dissolve

    me "Знаете, если бы Вы хоть попробовали, то может мы бы здесь не сидели." with dissolve

    me "Хоть я и знаю её несколько дней, но..." with dissolve

    me "Алия показала мне, что в даже в столь юной девушки может быть стальная воля, могут быть прекрасные мечты." with dissolve
...
label day4_parents_part0_choice_for_parents:
    ...
    me "Конечно, понимаю, что мне нужно было пресечь её глупую затею пойти против семьи," with dissolve

    me "но она была так серьёзно настроена, что я не рискнул." with dissolve
    ...
    me "Я взрослый мужчина и успел за свою жизнь многое повидать," with dissolve

    me "поэтому я прекрасно понимал, насколько опасно может быть подобная затея, для юной девушки." with dissolve
    ...
    me "Так, что я решил вызваться помочь, так я по крайней мере мог убедиться," with dissolve

    me "что она будет в порядке и не встрянет в какую-то опасную историю." with dissolve

    if day3_address_told:
        me "И именно поэтому, когда Вы позвонили я и сказал Вам адрес," with dissolve

        me "я был рад, что уважаемый Имран приехал за Алией и теперь она сможет вернуться к семье." with dissolve
    else:
