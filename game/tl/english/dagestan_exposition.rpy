# game/dagestan_exposition.rpy:31
translate english day1_ask_coach_dagestan_7a246719:

    # coach "Хорошо, с чего бы начать... Дагестан это национальная республика в составе России. Дагестан находится на Северном Кавказе" with dissolve
    coach "Well, how to start then... Dagestan is a national republic of Russia. Dagestan is located in the North Caucasus region" with dissolve

# game/dagestan_exposition.rpy:35
translate english day1_ask_coach_dagestan_6901729c:

    # coach "Столица - Махачкала. Население - три миллиона человек" with dissolve
    coach "Capital is Makhachkala. Population - three million" with dissolve

# game/dagestan_exposition.rpy:39
translate english day1_ask_coach_dagestan_90d15be2:

    # coach "Хотя Дагестан находится в составе России, жизнь в Дагестане сильно отличается от жизни в средней российской глубинке" with dissolve
    coach "Although Dagestan is located in Russia, living in Dagestan is quite different comparing to the living in the average Russian province" with dissolve

# game/dagestan_exposition.rpy:43
translate english day1_ask_coach_dagestan_d147c57a:

    # coach "Это же можно сказать и про другие республики Кавказа. В этом смысле они похожи" with dissolve
    coach "Same can be said about other national republics in the North Caucasus region. They are similar to each other in this case" with dissolve

# game/dagestan_exposition.rpy:47
translate english day1_ask_coach_dagestan_7c8ade84:

    # me "А чем Дагестан выделяется?" with dissolve
    me "What makes Dagestan different from them?" with dissolve

# game/dagestan_exposition.rpy:51
translate english day1_ask_coach_dagestan_22c7bbd2:

    # coach "Да, есть отличия, конечно же" with dissolve
    coach "Yes, of course, there are many differences" with dissolve

# game/dagestan_exposition.rpy:55
translate english day1_ask_coach_dagestan_751e7beb:

    # coach "Во-первых, Дагестан самый населенный - три миллиона жителей, первое место среди всех субъектов на Северном Кавказе" with dissolve
    coach "First of all, Dagestan has the highest population - 3 million, first place between other North Caucasus republics and provinces" with dissolve

# game/dagestan_exposition.rpy:59
translate english day1_ask_coach_dagestan_9cf5f567:

    # coach "Во-вторых, Дагестан - очень многонациональная республика" with dissolve
    coach "Second, Dagestan is a very multinational republic" with dissolve

# game/dagestan_exposition.rpy:63
translate english day1_ask_coach_dagestan_bbe3ff79:

    # coach "Если в Чечне преимущественно живут чеченцы, в Ингушетии - ингуши и так далее, то в Дагестане вместе проживают более полтора десятка национальностей" with dissolve
    coach "In Chechnya, most of the polulation are Chechens, in Ingushetia, most are Ingushs, and so on, but in Dagestan, there are more than fifteen nationalities live together" with dissolve

# game/dagestan_exposition.rpy:67
translate english day1_ask_coach_dagestan_ef3a4f84:

    # coach "Поэтому про дагестанца говорить, что \"он по национальности - дагестанец\", это неверно" with dissolve
    coach "That's why if you say about Dagestanian \"his nationality is Dagestanian\", that would be incorrect" with dissolve

# game/dagestan_exposition.rpy:71
translate english day1_ask_coach_dagestan_825fc118:

    # coach "\"Дагестанец\" может быть по национальности аварцем, даргинцем, лезгином, лакцем или даже русским" with dissolve
    coach "\"Dagestanian\" might be Avar, Dargin, Lezgian, Laks or even Russian" with dissolve

# game/dagestan_exposition.rpy:75
translate english day1_ask_coach_dagestan_8d486b2f:

    # coach "У каждого из народов Дагестана свой язык, непохожий на другие" with dissolve
    coach "Each Dagestan natinality has its own language, different from others" with dissolve

# game/dagestan_exposition.rpy:79
translate english day1_ask_coach_dagestan_bee47691:

    # coach "Может быть и так, что выходцы из одного села в Дагестане не будут понимать язык жителей другого села" with dissolve
    coach "It might happen so native of one Dagestanian village will not understand language of natives of another village" with dissolve

# game/dagestan_exposition.rpy:83
translate english day1_ask_coach_dagestan_21eefc82:

    # coach "Особенно сильно это различие в языках проявляется в Махачкале, где все народы плотно перемешались и живут вместе" with dissolve
    coach "Particulatly, this language difference shows up in Makhachkala, where all nations tightly mixed up and live together" with dissolve

# game/dagestan_exposition.rpy:87
translate english day1_ask_coach_dagestan_1f643bd9:

    # coach "В этом случае дагестанцы, чтобы понимать друг друга, переходят на русский язык. Русский язык становится языком межнационального общения" with dissolve
    coach "In this case, Dagestanians, in order to understand each other, use Russian. Russian becomes a lingua franka for them" with dissolve

# game/dagestan_exposition.rpy:91
translate english day1_ask_coach_dagestan_d76442c8:

    # coach "Поэтому средний дагестанец достаточно хорошо знает русский язык, а если взять жителя Махачкалы - то и использует его в ежедневном общений" with dissolve
    coach "That is why a regular Dagestanian knows Russian quite well, and if you take residents of Makhachkala - they use it in every day conversation" with dissolve

# game/dagestan_exposition.rpy:95
translate english day1_ask_coach_dagestan_21e9efdb:

    # coach "Еще одна особенность - по сравнению с остальным Кавказом, в Махачкале достаточно либеральные нравы" with dissolve
    coach "Another thing that makes difference comparing to other Caucasus, is that Makhachkala is a quite liberal city" with dissolve

# game/dagestan_exposition.rpy:99
translate english day1_ask_coach_dagestan_11bcb344:

    # coach "Девушки могут ходить в чем хотят. Кто-то конечно закрывает волосы платком, но немало девушек ходят джинсах и в юбках" with dissolve
    coach "Women can wear whatever they want. Some women, of course, cover their head with headscarf, but also many of them wear jeans and skirts" with dissolve

# game/dagestan_exposition.rpy:103
translate english day1_ask_coach_dagestan_f94fd0ac:

    # coach "Представить что-то подобное в Грозном, например, просто немыслимо" with dissolve
    coach "It is impossible to imagine something like that in Grozny, for example" with dissolve

# game/dagestan_exposition.rpy:107
translate english day1_ask_coach_dagestan_72995799:

    # coach "Более того, в Махачкале проходит ежегодный фестиваль аниме-культуры, где встречаются косплееры, фанаты аниме и видеоигр" with dissolve
    coach "Even more, Every year Makhachkala holds an Anime culture festival, where cosplayers, gamers and anime fans meet up" with dissolve

# game/dagestan_exposition.rpy:111
translate english day1_ask_coach_dagestan_31c3109c:

    # coach "Правда, в прошлом году фестиваль был сорван толпой агрессивно настроенных молодых людей" with dissolve
    coach "However, last year a group of agressive young men disrupted Anime culture festival" with dissolve

# game/dagestan_exposition.rpy:115
translate english day1_ask_coach_dagestan_7b065d27:

    # coach "Но все равно, сам факт. Если сравнить например с Чечней, то разница значительная" with dissolve
    coach "But even if taking this into consideration, you can feel the difference, for example, to Chechnya" with dissolve

# game/dagestan_exposition.rpy:119
translate english day1_ask_coach_dagestan_2ead5189:

    # me "Интересно, спасибо!" with dissolve
    me "Interesting, thanks!" with dissolve

# game/dagestan_exposition.rpy:123
translate english day1_ask_coach_dagestan_6903d792:

    # coach "Если тебя интересует какая-то конкретная тема, то спрашивай! Я расскажу более подробно" with dissolve
    coach "If you want to ask about some particulay topic, go ahead! I will tell more details" with dissolve

# game/dagestan_exposition.rpy:158
translate english day1_ask_coach_danger_109416b4:

    # me "В Дагестане опасно?" with dissolve
    me "It is dangerous in Dagestan?" with dissolve

# game/dagestan_exposition.rpy:162
translate english day1_ask_coach_danger_b973b544:

    # coach "Нет, там безопасно, как и в других регионах Северного Кавказа" with dissolve
    coach "No, it is safe there. Same as in other North Caucasus republics" with dissolve

# game/dagestan_exposition.rpy:166
translate english day1_ask_coach_danger_9c771905:

    # coach "Уровень насильственных преступлений там очень низкий по сравнению с другими регионами России" with dissolve
    coach "Violent crime rate there is very low comparing to other provinces in Russia" with dissolve

# game/dagestan_exposition.rpy:170
translate english day1_ask_coach_danger_ef577399:

    # coach "По улицам можно ходить ночью не опасаясь, что тебя ограбят или убьют" with dissolve
    coach "You can safely go outside at night with no fear that you will be robbed or killed" with dissolve

# game/dagestan_exposition.rpy:174
translate english day1_ask_coach_danger_c2a79dee:

    # me "А что насчет террористической угрозы?" with dissolve
    me "What about the terrorist threat?" with dissolve

# game/dagestan_exposition.rpy:178
translate english day1_ask_coach_danger_10a02c47:

    # coach "Террористическая угроза существовала в нулевых годах. Было время, боевики спускались с гор и нападали на блокпосты" with dissolve
    coach "There was the terrorist threat in the beginning of the century. There was a time when terrorists were coming down from mountains and attacking the checkpoints" with dissolve

# game/dagestan_exposition.rpy:182
translate english day1_ask_coach_danger_d0c78e84:

    # coach "Однако примерно с 2012 года, радикально настроенные экстремисты начали уезжать воевать в Сирию" with dissolve
    coach "However, starting from around 2012, radical extremists started to move to Syria and fight there" with dissolve

# game/dagestan_exposition.rpy:186
translate english day1_ask_coach_danger_2bd633e3:

    # coach "На текущий момент, в горах Дагестана больше нет никаких бандформирований" with dissolve
    coach "As for now, there is no terrorist gangs in the mountains" with dissolve

# game/dagestan_exposition.rpy:190
translate english day1_ask_coach_danger_291af573:

    # coach "Сейчас самое спокойное время в Дагестане, можно сказать, за всю его современную историю" with dissolve
    coach "You can say, right now is the safest historical period in Dagestan among ages" with dissolve

# game/dagestan_exposition.rpy:194
translate english day1_ask_coach_danger_a2acd505:

    # me "Понял" with dissolve
    me "I got it" with dissolve

# game/dagestan_exposition.rpy:205
translate english day1_ask_coach_religion_ec49a381:

    # me "Расскажи про религию в Дагестане" with dissolve
    me "Tell me about religion in Dagestan" with dissolve

# game/dagestan_exposition.rpy:209
translate english day1_ask_coach_religion_ac4f164c:

    # coach "В Дагестане большинство населения - мусульмане-сунниты" with dissolve
    coach "Dagestanians are largely Sunni Muslims" with dissolve

# game/dagestan_exposition.rpy:213
translate english day1_ask_coach_religion_4d6dc63f:

    # coach "Если ты не различаешь суннитов и шиитов, то для тебя это не существенно, можешь не запоминать" with dissolve
    coach "If you don't know the difference between Sunni and Shia, you don't have to remember that, it does not matter" with dissolve


# game/dagestan_exposition.rpy:217
translate english day1_ask_coach_religion_34ba0cbd:

    # coach "Несмотря на то, что большинство населения исповедуют ислам, в Дагестане не принято быть очень уж религиозным" with dissolve
    coach "Although most of the population adheres to Islam, it is not encouraged to be too much religious" with dissolve

# game/dagestan_exposition.rpy:221
translate english day1_ask_coach_religion_2a578de1:

    # coach "Когда дагестанец молится и соблюдает запреты Шариата это нормально" with dissolve
    coach "If a Dagestanian prays and follow the Sharia restrictions, that is ok" with dissolve

# game/dagestan_exposition.rpy:225
translate english day1_ask_coach_religion_8308a730:

    # coach "Но, если он начинает учить арабский язык, изучать какие-то тексты, литературу - на него начинают косо смотреть и подозревать в экстремизме" with dissolve
    coach "But if he starts to learn Arabic langauge, study some texts, literature - others start to look askance at him and suspect in extremism" with dissolve

# game/dagestan_exposition.rpy:229
translate english day1_ask_coach_religion_829ffc89:

    # coach "Это в какой-то мере идет еще с советских времен. И это особенно характерно для силовиков, или просто госслужащих" with dissolve
    coach "To some extent, that comes since Soviet Union times. Particularly, this is common for military, police, or just civil servants" with dissolve

# game/dagestan_exposition.rpy:233
translate english day1_ask_coach_religion_027c981b:

    # coach "Доходит до того, что дагестанцы стараются быть чисто выбритыми, чтобы не быть похожими на бородачей-террористов" with dissolve
    coach "Even some Dagestanians try to be clean-shaven, to not look like terrorists wearing beards" with dissolve

# game/dagestan_exposition.rpy:237
translate english day1_ask_coach_religion_3d659808:

    # coach "Это относится больше к взрослому поколению, в особенности тех, кто еще застал Советский союз" with dissolve
    coach "This mostly related to adult generation, particularly those who was living during the Soviet Union" with dissolve

# game/dagestan_exposition.rpy:241
translate english day1_ask_coach_religion_cff9066b:

    # coach "Современная молодежь в Дагестане в среднем намного более религиозна, чем их родители" with dissolve
    coach "Modern youth generation in Dagestan, in average, is more religious than their parents" with dissolve

# game/dagestan_exposition.rpy:245
translate english day1_ask_coach_religion_cb097cfe:

    # coach "Но опять же, все дагестанцы разные, и поэтому нельзя что-то сказать однозначное. Кто-то более религиозен, а кто-то менее" with dissolve
    coach "But again, all Dagestanians are different, and you can't say something definitely. Some people are more religous, some are less" with dissolve

# game/dagestan_exposition.rpy:249
translate english day1_ask_coach_religion_22594686:

    # me "Понятно" with dissolve
    me "Ok, I got it" with dissolve

# game/dagestan_exposition.rpy:259
translate english day1_ask_coach_tradition_4e5df096:

    # me "Расскажи про традиции в Дагестане" with dissolve
    me "Tell me about traditions in Dagestan" with dissolve

# game/dagestan_exposition.rpy:263
translate english day1_ask_coach_tradition_8674aea7:

    # coach "Как и на всем Северном Кавказе, в Дагестане сильны традиции гостеприимства" with dissolve
    coach "As in the whole North Caucasus, hospitalitiy traditions are strong in Dagestan" with dissolve

# game/dagestan_exposition.rpy:267
translate english day1_ask_coach_tradition_43682b5c:

    # coach "В мелких городах и селах там почти нет гостиниц, потому что гостиницы не нужны" with dissolve
    coach "In small cities and villages, there are almost no hotels. Because hotels are not needed" with dissolve

# game/dagestan_exposition.rpy:271
translate english day1_ask_coach_tradition_dadaaa90:

    # coach "Если ты приехал издалека можешь просто подойти к любому дому, постучаться и попроситься переночевать. Тебя пустят, накормят и дадут ночлег" with dissolve
    coach "If you come from far away, you just pick any house, knock in and ask for an overnight stay. They will let you in, offer some meal and give a place to sleep" with dissolve

# game/dagestan_exposition.rpy:275
translate english day1_ask_coach_tradition_fed2135a:

    # coach "В Дагестане принято уважение к старшим. Старшим нужно уступать места, старших нужно слушаться" with dissolve
    coach "In Dagestan, a respect for elders is common. One must yield a seat to an elder, one must obey an elder" with dissolve

# game/dagestan_exposition.rpy:279
translate english day1_ask_coach_tradition_7a21eb00:

    # coach "Особенно сильно это проявляется в семье. Ослушаться отца — это немыслимое дело!" with dissolve
    coach "Especially, it shows up inside the family. It is unthinkable to disobey one's father!" with dissolve

# game/dagestan_exposition.rpy:283
translate english day1_ask_coach_tradition_2fcfda67:

    # coach "Дагестанец может не бояться ни драки, ни полиции, и единственное чего он боится — это потерять уважение своего отца" with dissolve
    coach "A Dagestanian might not fear the fight, or the police, and the only thing he fears - is to loose his father's respect" with dissolve

# game/dagestan_exposition.rpy:287
translate english day1_ask_coach_tradition_69f1c8bb:

    # coach "Еще в Дагестане достаточно трепетное отношение к своим женщинам" with dissolve
    coach "Also, in Dagestan, there is reverent attitude to their women" with dissolve

# game/dagestan_exposition.rpy:291
translate english day1_ask_coach_tradition_43f9b24c:

    # coach "Женщина в Дагестане находится под защитой всех своих родственников" with dissolve
    coach "A Dagestanian women is considered to be under protection of all her relatives" with dissolve

# game/dagestan_exposition.rpy:295
translate english day1_ask_coach_tradition_521e54cc:

    # coach "Не стоит обижать дагестанку - она пожалуется, например, своему брату, который вызовет обидчика на разговор" with dissolve
    coach "One should not insult a Dagestanian woman - she will complain to her brother, for example, who will ask the offender to have a talk" with dissolve

# game/dagestan_exposition.rpy:299
translate english day1_ask_coach_tradition_512e9465:

    # coach "По этой же причине в Дагестане не принято даже делать замечание посторонней девушке" with dissolve
    coach "That is why, it is not common in Dagestan to make a comment to a woman you don't know" with dissolve

# game/dagestan_exposition.rpy:303
translate english day1_ask_coach_tradition_2fb92406:

    # coach "Это будет невежливо, оскорбительно, и тот, кто сделал замечание, может получить последствия" with dissolve
    coach "That would be impolite, offensive, and the one who says a comment, might get consequences" with dissolve

# game/dagestan_exposition.rpy:307
translate english day1_ask_coach_tradition_d82531f3:

    # coach "Если дагестанец видит, что девушка ведет себя неподобающе, он скорее всего промолчит, максимум - скажет об этом ее родственнику мужского пола" with dissolve
    coach "If a Dagestanian sees that a woman behave inappropriately, most likely he will keep silence. At most - he will tell about it to her male relative" with dissolve

# game/dagestan_exposition.rpy:311
translate english day1_ask_coach_tradition_b0da3220:

    # me "Ага" with dissolve
    me "Okay" with dissolve

# game/dagestan_exposition.rpy:326
translate english day1_ask_coach_woman1_3fb33ff6:

    # me "Расскажи про положение женщины в обществе Дагестана" with dissolve
    me "Tell me about women in the Dagestan society" with dissolve


# game/dagestan_exposition.rpy:329
translate english day1_ask_coach_woman1_864bf76d:

    # coach "Если смотреть в целом на государственном уровне, то все выглядит прилично" with dissolve
    coach "If you see in general at the state level, everything looks fine" with dissolve

# game/dagestan_exposition.rpy:333
translate english day1_ask_coach_woman1_3f2f1e18:

    # coach "Декларируется равноправие, никаких ограничений нет, женщины в Дагестане учатся в колледжах и в университетах" with dissolve
    coach "Equality is declared, there are no restrictions for women, women in Dagestan study in colleges and universities" with dissolve

# game/dagestan_exposition.rpy:337
translate english day1_ask_coach_woman1_9c419ff6:

    # coach "Женщины получают профессию и работают наравне с мужчинами" with dissolve
    coach "Women get a profession and work alongside with men" with dissolve

# game/dagestan_exposition.rpy:341
translate english day1_ask_coach_woman1_a653064b:

    # coach "Тем не менее, общество в Дагестане - патриархальное. Женщина зачастую воспринимается не как самостоятельная личность, а как принадлежащая своему мужу или отцу" with dissolve
    coach "However, the society in Dagestan is patriarchal. A woman is often considered not as an independent individual, but as she belongs to her husband or father" with dissolve

# game/dagestan_exposition.rpy:345
translate english day1_ask_coach_woman1_6a8e57f6:

    # coach "И тут все зависит уже от семьи. Если родители девушки либеральные, они не будут ограничивать свою дочь" with dissolve
    coach "And at this point, everything depends on the particular family. If her parents are broad-minded, they will not limit their daughter" with dissolve

# game/dagestan_exposition.rpy:349
translate english day1_ask_coach_woman1_1dd6c0c7:

    # coach "Они позволят ей учиться там, где она хочет, позволят ей замуж за кого она хочет" with dissolve
    coach "They will let her study where she wants, let her marry whoever she wants" with dissolve

# game/dagestan_exposition.rpy:353
translate english day1_ask_coach_woman1_2b383a1e:

    # coach "Но если девушка выросла где-нибудь в селе, то скорее всего родители не будут считаться с ее мнением" with dissolve
    coach "But if a girl grew up in some village, then most likely her parents will not consider her opinion" with dissolve

# game/dagestan_exposition.rpy:357
translate english day1_ask_coach_woman1_fb94e0c9:

    # coach "Девушка будет вынуждена выйти замуж за того, кого родители посчитают достойным женихом" with dissolve
    coach "A girl would be forced to marry the one who parents would consider a perspective fiance" with dissolve

# game/dagestan_exposition.rpy:361
translate english day1_ask_coach_woman1_7cfa7a1d:

    # coach "И в дальнейшем девушка будет вынуждена жить с мужем и слушаться мужа" with dissolve
    coach "And in the future a girl would have to live with her husband and obey him" with dissolve

# game/dagestan_exposition.rpy:366
translate english day1_ask_coach_woman1_9d40a2c2:

    # coach "Дагестан меняется со временем. Но и сейчас по-прежнему существуют убийства чести и свадьбы по принуждению" with dissolve
    coach "Dagestan is changing over time. But still, even nowadays there are honor killings and forced marriages" with dissolve

# game/dagestan_exposition.rpy:370
translate english day1_ask_coach_woman1_aedfae6e:

    # coach "В-общем, тема обширная. Что именно ты хочешь узнать?" with dissolve
    coach "Generally, the topic is broad. What exactly do you want to know?" with dissolve

# game/dagestan_exposition.rpy:406
translate english day1_ask_coach_woman_decline_marry_82fa7153:

    # me "Что будет если девушка откажется выходить замуж?" with dissolve
    me "What if a girl refuse to marry a fiance?" with dissolve

# game/dagestan_exposition.rpy:409
translate english day1_ask_coach_woman_decline_marry_f6a9712c:

    # coach "Если девушка живет в селе, то особо выбора-то и нет" with dissolve
    coach "If a girl live in the village, then there is no choice, basically" with dissolve

# game/dagestan_exposition.rpy:413
translate english day1_ask_coach_woman_decline_marry_4e7f0b71:

    # coach "Она живет в селе, и все односельчане живут по заведенным традициям, и она не представляет, как можно жить иначе" with dissolve
    coach "She lives in the village, and all fellow villagers live by traditions, and she can't imagine how to live different life" with dissolve

# game/dagestan_exposition.rpy:417
translate english day1_ask_coach_woman_decline_marry_eb6bc8d2:

    # coach "Если девушке прямо совсем неприятен мужчина, за которого она выходит замуж, то выбор невелик" with dissolve
    coach "If a fiance is really disgusting for her, there are only a few options" with dissolve

# game/dagestan_exposition.rpy:421
translate english day1_ask_coach_woman_decline_marry_5a8ee014:

    # coach "Она может сбежать из дома или совершить самоубийство. И то, и другое сложно сделать, как ты понимаешь" with dissolve
    coach "She can run away from home or commit suicide. Both options are difficult to do, as you understand" with dissolve

# game/dagestan_exposition.rpy:426
translate english day1_ask_coach_woman_decline_marry_1262c283:

    # coach "Если же дагестанка, допустим, училась в Махачкале, вкусила городской жизни, имеет широкий круг друзей и интересов" with dissolve
    coach "Let's say, a Dagestanian woman, for example, studied in Makhachkala, tasted ubran life, have a wide cirlce of friends and interests" with dissolve

# game/dagestan_exposition.rpy:430
translate english day1_ask_coach_woman_decline_marry_df96cf68:

    # coach "Вот она получила диплом, собирается устроиться на работу, и тут ее вдруг родители хотят вернуть в село и выдать замуж за односельчанина" with dissolve
    coach "She graduated, she prepares to get a job, and then suddenly parents want her to come back to her village and marry some fellow villager." with dissolve

# game/dagestan_exposition.rpy:433
translate english day1_ask_coach_woman_decline_marry_01228453:

    # coach "Вот тут уже не все так однозначно" with dissolve
    coach "Then it starts getting ambiguous" with dissolve

# game/dagestan_exposition.rpy:437
translate english day1_ask_coach_woman_decline_marry_553f5641:

    # coach "Может быть, девушке удастся отстоять свою независимость и переубедить родителей" with dissolve
    coach "Probably, a woman might stand for her independence and persuade her parents" with dissolve

# game/dagestan_exposition.rpy:441
translate english day1_ask_coach_woman_decline_marry_91ad206d:

    # coach "А может быть, договориться с родителями не удастся, и тогда возможен любой исход, вплоть до убийства чести" with dissolve
    coach "Or might be they will not be able to agree, and then there might be any outcome, up to the honor killing" with dissolve

# game/dagestan_exposition.rpy:445
translate english day1_ask_coach_woman_decline_marry_ad028ae1:

    # me "Да уж" with dissolve
    me "Ok, I got it" with dissolve


# game/dagestan_exposition.rpy:457
translate english day1_ask_coach_woman_love_b76bb3a0:

    # me "Что делать, если девушка полюбила парня?" with dissolve
    me "What do they do if a woman fell in love with a man?" with dissolve

# game/dagestan_exposition.rpy:460
translate english day1_ask_coach_woman_love_c0940e2b:

    # coach "По правилам, парень и девушка не должны \"встречаться\", в том смысле как это происходит у русских" with dissolve
    coach "According to the rules, a man and a woman should not \"date\", meaning how it usually is for Russians" with dissolve

# game/dagestan_exposition.rpy:464
translate english day1_ask_coach_woman_love_ec48e8b1:

    # coach "Если парню понравилась девушка, он должен посвататься - просить руки и сердца у ее родителей" with dissolve
    coach "If a man likes a woman, he should come to her father and ask for a permission to marry her daughter" with dissolve

# game/dagestan_exposition.rpy:468
translate english day1_ask_coach_woman_love_6bfa8300:

    # coach "Если родители одобрят, то парень сможет взять девушку в жены" with dissolve
    coach "If parents approve him, then he can marry a woman" with dissolve

# game/dagestan_exposition.rpy:473
translate english day1_ask_coach_woman_love_8675bc6e:

    # coach "Если вместо этого девушка будет ходить на свидания с парнем, то она будет считаться распутной" with dissolve
    coach "If instead of that a woman would go on a date with a boy, she would be considered slutty" with dissolve

# game/dagestan_exposition.rpy:476
translate english day1_ask_coach_woman_love_c2c0a1b0:

    # coach "Разумеется, конкретные рамки допустимого могут варьироваться. В Махачкале студентка ДГУ может без проблем общаться со своими одногруппниками" with dissolve
    coach "Of course, exact limitations may vary. In Makhachkala, a female student of Dagestan State Univesity can freely conversate with her classmates" with dissolve

# game/dagestan_exposition.rpy:480
translate english day1_ask_coach_woman_love_10bac506:

    # coach "в то время как где-нибудь в селе девушка не осмелится даже заговорить с незнакомым мужчиной" with dissolve
    coach "And at the same time, somewhere in village a girl may not even dare to talk to an unfamiliar man" with dissolve

# game/dagestan_exposition.rpy:484
translate english day1_ask_coach_woman_love_9dd8300b:

    # coach "Однозначно существует лишь запрет на ночевку в доме с чужим мужчиной. Все остальное - по ситуации" with dissolve
    coach "Definitely, it is resticted to stay overnight with other man. Other restrictions may vary" with dissolve

# game/dagestan_exposition.rpy:488
translate english day1_ask_coach_woman_love_4d4bb506:

    # me "Интересно" with dissolve
    me "Interesting" with dissolve

# game/dagestan_exposition.rpy:500
translate english day1_ask_coach_woman_honor_killing_6de58e80:

    # me "Что такое убийство чести?" with dissolve
    me "What is a honor killing?" with dissolve

# game/dagestan_exposition.rpy:503
translate english day1_ask_coach_woman_honor_killing_0daa6c97:

    # coach "На Кавказе конечно так не говорят, там говорят \"смыть позор\"" with dissolve
    coach "Of course, in Caucasus they don't say like that, they say \"to wash out the disgrace\"" with dissolve

# game/dagestan_exposition.rpy:507
translate english day1_ask_coach_woman_honor_killing_50e8b51d:

    # coach "Если девушка ведет себя неподобающе, то считается что она таким образом позорит весь свой род" with dissolve
    coach "If a woman behave inappropriately, it is considered that she puts a shame on all her relatives" with dissolve

# game/dagestan_exposition.rpy:511
translate english day1_ask_coach_woman_honor_killing_c04fd294:

    # coach "Кто-то из родственников мужского пола может в таком случае решить убить ее, чтобы решить проблему" with dissolve
    coach "In this case, some of male relatives may decide to kill her to solve the problem" with dissolve

# game/dagestan_exposition.rpy:515
translate english day1_ask_coach_woman_honor_killing_f8be4f2f:

    # coach "Обычно у родителей и братьев рука не поднимется убить свою дочь, свою сестру, однако это может сделать кто-то из более дальних родственников" with dissolve
    coach "Usually, a father or a brother does not dare to kill their own daughter, their own sister, but some distant relative may do it" with dissolve

# game/dagestan_exposition.rpy:519
translate english day1_ask_coach_woman_honor_killing_aeb96da0:

    # coach "Обычно убивают за измену, за блуд, за отказ выходить замуж. Реже - за слухи" with dissolve
    coach "Usually a woman is being killed for cheating, for fornication, for marriage refusal. Less often - because of rumors" with dissolve

# game/dagestan_exposition.rpy:523
translate english day1_ask_coach_woman_honor_killing_e8d657a8:

    # coach "Настоящие убийства чести — это все же редкое явление и служат скорее \"пугалом\"" with dissolve
    coach "Real honor killing is actually very rarely happen, and usually works as a \"scarecrow\"" with dissolve

# game/dagestan_exposition.rpy:528
translate english day1_ask_coach_woman_honor_killing_6b6c8470:

    # coach "Истории об убийствах чести циркулируют в обществе, в виде тем, которые люди обсуждают, или даже в виде слухов" with dissolve
    coach "Stories about honor killings are being rotated in the society, as discussion topics or even as rumors" with dissolve

# game/dagestan_exposition.rpy:531
translate english day1_ask_coach_woman_honor_killing_9d461323:

    # coach "Но в реальной жизни обычно такие вопросы все-таки решаются без крови" with dissolve
    coach "But in reality, usually this kind of questions are solved without blood" with dissolve

# game/dagestan_exposition.rpy:536
translate english day1_ask_coach_woman_honor_killing_22594686:

    # me "Понятно" with dissolve
    me "Ok, I got it" with dissolve

# game/dagestan_exposition.rpy:547
translate english day1_ask_coach_woman_kidnapping_44c9af41:

    # me "Расскажи про традицию похищения невесты" with dissolve
    me "Tell me about bride kidnapping tradition" with dissolve

# game/dagestan_exposition.rpy:550
translate english day1_ask_coach_woman_kidnapping_87c27832:

    # coach "Похищение невесты в Дагестане все-таки осуждается" with dissolve
    coach "Bride kidnapping in Dagestan is actually condemned" with dissolve

# game/dagestan_exposition.rpy:554
translate english day1_ask_coach_woman_kidnapping_16039559:

    # coach "Если парню понравилась девушка, будет правильнее всего идти к ее родителям, просить руки и сердца" with dissolve
    coach "If a man likes a woman, it would be right to visit her parents and ask to marry her" with dissolve

# game/dagestan_exposition.rpy:558
translate english day1_ask_coach_woman_kidnapping_60c377c1:

    # coach "Похищение имеет смысл лишь в том случае, если не удается договориться нормально" with dissolve
    coach "Bride kidnapping might happen if they could not set an agreement" with dissolve

# game/dagestan_exposition.rpy:562
translate english day1_ask_coach_woman_kidnapping_1b60a093:

    # coach "Это может быть, когда парень и девушка любят друг друга, но родители против свадьбы" with dissolve
    coach "It might happen if a man and woman love each other, but her parents are against their marriage" with dissolve

# game/dagestan_exposition.rpy:566
translate english day1_ask_coach_woman_kidnapping_55bcdac0:

    # coach "Второй вариант, похуже - парень хочет взять девушку в жены, но девушка против, поэтому и родители против" with dissolve
    coach "Another case, much worse - if a man wants to marry a woman, but the woman refuse, so her parents also refuse" with dissolve

# game/dagestan_exposition.rpy:570
translate english day1_ask_coach_woman_kidnapping_b4111671:

    # coach "В первом случае, парень и девушка заранее договариваются о похищении, поэтому похищение проходит скорее для вида" with dissolve
    coach "In first case, man and woman set up the kindapping in advance, so it looks like \"kindapping\" just for others" with dissolve

# game/dagestan_exposition.rpy:574
translate english day1_ask_coach_woman_kidnapping_38e3ecdc:

    # coach "Но может быть, что девушка реально не хочет выходить за парня, родители также против, но жених очень хочет взять в жены именно ее" with dissolve
    coach "But in the second case, when a woman really don't want to marry a man, and her parents are also against it, but the man want to marry only her" with dissolve

# game/dagestan_exposition.rpy:578
translate english day1_ask_coach_woman_kidnapping_96e1f961:

    # coach "Тогда жених действительно может организовать похищение, и довольно жестокое" with dissolve
    coach "Then the man can really set up a bride kidnapping, and quite brutal" with dissolve

# game/dagestan_exposition.rpy:582
translate english day1_ask_coach_woman_kidnapping_31c28672:

    # coach "Девушку завернут в бурку, кинут в автомобиль и отвезут в дом жениха" with dissolve
    coach "A woman will be wrapped into burka, thrown in the car and delivered to the fiance's home" with dissolve

# game/dagestan_exposition.rpy:586
translate english day1_ask_coach_woman_kidnapping_03bd273d:

    # coach "Дальше, если девушка провела ночь в доме мужчины, родителям приходится согласится выдать замуж девушку за этого мужчину, под давлением общественного мнения" with dissolve
    coach "Later on, if a woman spent the night in the man's home, her parents are obliged to consent to the marriage, under public opinion pressure" with dissolve

# game/dagestan_exposition.rpy:590
translate english day1_ask_coach_woman_kidnapping_a79ea649:

    # me "Ясно" with dissolve
    me "Ok, ths is clear for me" with dissolve

# game/dagestan_exposition.rpy:601
translate english day1_ask_coach_knowall_12736858:

    # me "Да, я знаю о Дагестане и о традициях Дагестана" with dissolve
    me "Yes, I know about Dagestan and traditions of Dagestan" with dissolve

# game/dagestan_exposition.rpy:610
translate english day1_ask_coach_learnedall_8756cc2f:

    # me "Спасибо, я узнал все что мне нужно" with dissolve
    me "Thank you, I learned everything I need" with dissolve

# game/dagestan_exposition.rpy:619
translate english day1_ask_coach_finish_972fecc3:

    # coach "Отлично!" with dissolve
    coach "Great!" with dissolve

translate english strings:

    # dagestan_exposition.rpy:12
    old "Что ответить?"
    new "How to reply?"

    # dagestan_exposition.rpy:12
    old "\"Почти ничего не знаю\""
    new "\"I know almost nothing\""

    # dagestan_exposition.rpy:12
    old "\"Да, я знаю о Дагестане и о традициях Дагестана\""
    new "\"Yes, I know about Dagestan and traditions of Dagestan\""

    # dagestan_exposition.rpy:132
    old "Что спросить?"
    new "What to ask?"

    # dagestan_exposition.rpy:132
    old "\"В Дагестане опасно?\""
    new "\"It is dangerous in Dagestan?\""

    # dagestan_exposition.rpy:132
    old "Религия в Дагестане"
    new "Religion in Dagestan"

    # dagestan_exposition.rpy:132
    old "Традиции Дагестана"
    new "Traditions in Dagestan"

    # dagestan_exposition.rpy:132
    old "Положение женщины в обществе Дагестана"
    new "Women in the Dagestan society"

    # dagestan_exposition.rpy:132
    old "\"Спасибо, я узнал все что мне нужно\""
    new "\"Thank you, I learned everything I need\""

    # dagestan_exposition.rpy:380
    old "\"Что будет если девушка откажется выходить замуж?\""
    new "\"What if a girl refuse to marry a fiance?\""

    # dagestan_exposition.rpy:380
    old "\"Что делать, если девушка полюбила парня?\""
    new "\"What do they do if a woman fell in love with a man?\""

    # dagestan_exposition.rpy:380
    old "\"Что такое убийство чести?\""
    new "\"What is a honor killing?\""

    # dagestan_exposition.rpy:380
    old "Похищение невесты"
    new "Bride kidnapping tradition"

    # dagestan_exposition.rpy:380
    old "Вернуться назад"
    new "Go back"
# TODO: Translation updated at 2021-12-11 15:30

# game/dagestan_exposition.rpy:10
translate english dagestan_exposition_a2b75fc5:

    # coach "Она из Дагестана. Ты много знаешь об этой республике?" with dissolve
    coach "She is from Dagestan. How much do you know about Dagestan?" with dissolve

# game/dagestan_exposition.rpy:27
translate english day1_ask_coach_dagestan_bedbe269:

    # me "Почти ничего не знаю." with dissolve
    me "I know almost nothing." with dissolve

# game/dagestan_exposition.rpy:31
translate english day1_ask_coach_dagestan_c380eb8a:

    # coach "Хорошо, с чего бы начать... Дагестан это национальная республика, которая находится на Северном Кавказе." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:35
translate english day1_ask_coach_dagestan_1cfe89e5:

    # coach "Хотя Дагестан находится в составе России, жизнь в нём сильно отличается от жизни в средней российской глубинке." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:39
translate english day1_ask_coach_dagestan_8ee7003e:

    # coach "Ну вообще, все республики на Кавказе - особенные, но Дагестан, наверное, по-своему особенная республика." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:43
translate english day1_ask_coach_dagestan_391df747:

    # me "Чем Дагестан выделяется?" with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:47
translate english day1_ask_coach_dagestan_6a93bda6:

    # coach "Во-первых, Дагестан самый населённый - три миллиона жителей - первое место среди всех субъектов Северного Кавказа." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:51
translate english day1_ask_coach_dagestan_ed0a322e:

    # coach "Во-вторых, это очень многонациональная республика." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:55
translate english day1_ask_coach_dagestan_46696bbb:

    # coach "Если в Чечне преимущественно живут чеченцы, в Ингушетии - ингуши и так далее, то в Дагестане вместе проживают более полутора десятка национальностей." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:59
translate english day1_ask_coach_dagestan_ba22949f:

    # coach "Поэтому говорить про дагестанца, что \"он по национальности дагестанец\" - неверно." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:63
translate english day1_ask_coach_dagestan_25abc1a9:

    # coach "\"Дагестанец\" может быть по национальности аварцем, даргинцем, лезгином, лакцем или даже русским." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:67
translate english day1_ask_coach_dagestan_d91185b5:

    # coach "У каждого из народов республики Дагестан свой язык, который не похож на другие." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:71
translate english day1_ask_coach_dagestan_fa822f12:

    # coach "Может быть и так, что выходцы из одного села в Дагестане не будут понимать язык жителей другого села." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:75
translate english day1_ask_coach_dagestan_bc7a7b8d:

    # coach "Особенно сильно это различие в языках проявляется в Махачкале, где все народы плотно перемешались и живут вместе." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:79
translate english day1_ask_coach_dagestan_54a26011:

    # coach "В этом случае дагестанцы, чтобы понимать друг друга, переходят на русский язык. Он становится языком межнационального общения." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:83
translate english day1_ask_coach_dagestan_94b4477a:

    # coach "Поэтому средний дагестанец достаточно хорошо знает русский язык, а если взять жителя Махачкалы, то и использует его в ежедневном общений." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:87
translate english day1_ask_coach_dagestan_ece69cf2:

    # coach "Ещё одна особенность: по сравнению с остальным Кавказом, в Махачкале достаточно либеральные нравы." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:91
translate english day1_ask_coach_dagestan_c9a30231:

    # coach "Девушки могут ходить в чём хотят: кто-то, конечно, закрывает волосы платком, но немало из них носит джинсы и юбки." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:95
translate english day1_ask_coach_dagestan_c45fbe1d:

    # coach "Представить что-то подобное в Грозном, например, просто немыслимо." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:99
translate english day1_ask_coach_dagestan_b3628444:

    # coach "Более того, в Махачкале проходит ежегодный фестиваль аниме-культуры, где встречаются косплееры, фанаты аниме и видеоигр." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:103
translate english day1_ask_coach_dagestan_c5e6cebb:

    # coach "Если сравнить, например, с Чечней или с Ингушетией - разница значительная." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:111
translate english day1_ask_coach_dagestan_52db1bb1:

    # coach "Спрашивай, если тебя интересует какая-то конкретная тема. Я расскажу более подробно." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:150
translate english day1_ask_coach_danger_0ea8fc1e:

    # coach "Нет, там безопасно как и в других регионах Северного Кавказа." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:154
translate english day1_ask_coach_danger_6240f8d7:

    # coach "Уровень насильственных преступлений очень низкий по сравнению с другими регионами России." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:158
translate english day1_ask_coach_danger_94a8a5a9:

    # coach "По улицам можно ходить ночью не опасаясь, что тебя ограбят или убьют." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:162
translate english day1_ask_coach_danger_cba87bd9:

    # me "А что насчёт террористической угрозы?" with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:166
translate english day1_ask_coach_danger_c03d3910:

    # coach "Террористическая угроза существовала в нулевых годах. Было время, боевики спускались с гор и нападали на блокпосты." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:170
translate english day1_ask_coach_danger_5cc4f0af:

    # coach "Однако, примерно с 2012 года, радикально настроенные экстремисты начали уезжать воевать в Сирию." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:174
translate english day1_ask_coach_danger_aef3c2a0:

    # coach "На текущий момент в горах Дагестана больше нет никаких бандформирований." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:178
translate english day1_ask_coach_danger_b902487f:

    # coach "Сейчас самое спокойное время в Дагестане, можно сказать, за всю его современную историю." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:182
translate english day1_ask_coach_danger_5bb0da84:

    # me "Понял." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:193
translate english day1_ask_coach_religion_8eaee815:

    # me "Расскажи про религию в Дагестане." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:197
translate english day1_ask_coach_religion_d5ba68b7:

    # coach "В Дагестане большинство населения - мусульмане-сунниты." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:201
translate english day1_ask_coach_religion_0247e26c:

    # coach "Если ты не различаешь суннитов и шиитов, то для тебя это не существенно. Можешь не запоминать." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:205
translate english day1_ask_coach_religion_637e4a37:

    # coach "Несмотря на то, что большинство населения исповедует ислам, в Дагестане не принято быть очень уж религиозным." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:209
translate english day1_ask_coach_religion_cd486e7a:

    # coach "Когда дагестанец молится и соблюдает запреты шариата это нормально." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:213
translate english day1_ask_coach_religion_9a75fc2f:

    # coach "Но стоит ему начать учить арабский язык, изучать какие-то тексты, литературу - на него начинают косо смотреть и подозревать в экстремизме." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:217
translate english day1_ask_coach_religion_f0997741:

    # coach "Это в какой-то мере идет ещё с советских времен и особенно характерно для силовиков или просто госслужащих." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:221
translate english day1_ask_coach_religion_23f7b951:

    # coach "Доходит до того, что дагестанцы стараются быть чисто выбритыми, чтобы не быть похожими на бородачей-террористов." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:225
translate english day1_ask_coach_religion_ac2e404c:

    # coach "Это относится больше к взрослому поколению, в особенности тех, кто ещё застал Советский Союз." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:229
translate english day1_ask_coach_religion_ab144e81:

    # coach "Современная молодёжь в Дагестане в среднем намного более религиозна, чем их родители." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:233
translate english day1_ask_coach_religion_955fd167:

    # coach "Но опять же, все дагестанцы разные и нельзя сказать что-то однозначное. Кто-то более религиозен, кто-то менее." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:237
translate english day1_ask_coach_religion_fe17819a:

    # me "Понятно." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:247
translate english day1_ask_coach_tradition_d3fe3ac8:

    # me "Расскажи про традиции в Дагестане." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:251
translate english day1_ask_coach_tradition_34a91c34:

    # coach "Как и на всём Северном Кавказе, в Дагестане сильны традиции гостеприимства." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:255
translate english day1_ask_coach_tradition_a4c02f9e:

    # coach "В мелких городах и сёлах почти нет гостиниц, потому что они не нужны." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:259
translate english day1_ask_coach_tradition_2df3a1fd:

    # coach "Если ты приехал издалека, можешь просто подойти к любому дому, постучаться и попроситься переночевать. Тебя пустят, накормят и дадут ночлег." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:263
translate english day1_ask_coach_tradition_fc055b45:

    # coach "В Дагестане принято уважать старших. Им нужно уступать места и слушаться." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:267
translate english day1_ask_coach_tradition_e16b1dad:

    # coach "Особенно сильно это проявляется в семье. Ослушаться отца — немыслимое дело!" with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:271
translate english day1_ask_coach_tradition_42d49401:

    # coach "Дагестанец может не бояться ни драки, ни полиции, единственное чего он боится — потерять уважение своего отца." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:275
translate english day1_ask_coach_tradition_773e86cc:

    # coach "Ещё в Дагестане достаточно трепетное отношение к своим женщинам." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:279
translate english day1_ask_coach_tradition_090bd0bf:

    # coach "Женщина в Дагестане находится под защитой всех своих родственников." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:283
translate english day1_ask_coach_tradition_b8289a49:

    # coach "Не стоит обижать дагестанку. Она пожалуется, например, своему брату, который вызовет обидчика на разговор." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:287
translate english day1_ask_coach_tradition_4bbb4dc2:

    # coach "По этой же причине в Дагестане не принято делать замечание посторонней девушке." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:291
translate english day1_ask_coach_tradition_3fdf67f6:

    # coach "Это будет невежливо, оскорбительно и тому, кто сделал замечание, может непоздоровиться." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:295
translate english day1_ask_coach_tradition_4704f7c9:

    # coach "Если дагестанец видит, что девушка ведёт себя неподобающе, он скорее всего промолчит, максимум - скажет об этом её родственнику-мужчине." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:299
translate english day1_ask_coach_tradition_a8fb0fbc:

    # me "Ага..." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:313
translate english day1_ask_coach_woman1_7e820d6b:

    # me "Расскажи про положение женщины в обществе Дагестана." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:317
translate english day1_ask_coach_woman1_5839c1c4:

    # coach "Если смотреть в целом на государственном уровне - всё выглядит прилично." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:321
translate english day1_ask_coach_woman1_9cea2bf5:

    # coach "Декларируется равноправие: никаких ограничений нет, женщины учатся в колледжах и университетах." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:325
translate english day1_ask_coach_woman1_5e90e1b3:

    # coach "Также они получают профессию и работают наравне с мужчинами." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:329
translate english day1_ask_coach_woman1_68bea83e:

    # coach "Тем не менее, общество в Дагестане патриархальное. Женщина зачастую воспринимается не как самостоятельная личность, а как принадлежащая своему мужу или отцу." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:333
translate english day1_ask_coach_woman1_bb506874:

    # coach "Тут всё зависит уже от семьи. Если родители девушки либеральные, они не будут ограничивать свою дочь." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:337
translate english day1_ask_coach_woman1_5ac25b60:

    # coach "Они позволят ей учиться там, где она пожелает и разрешат выйти замуж за кого она хочет." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:341
translate english day1_ask_coach_woman1_171703fd:

    # coach "Но если девушка выросла где-нибудь в селе, то скорее всего родители не будут считаться с её мнением." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:345
translate english day1_ask_coach_woman1_8af9a466:

    # coach "Девушка будет вынуждена выйти замуж за того, кого родители посчитают достойным женихом." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:349
translate english day1_ask_coach_woman1_92023565:

    # coach "И в дальнейшем она будет вынуждена жить с мужем и слушаться его." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:353
translate english day1_ask_coach_woman1_d2bc7755:

    # coach "Дагестан меняется со временем. Но и сейчас по-прежнему существуют убийства чести и свадьбы по принуждению." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:357
translate english day1_ask_coach_woman1_4d7981d8:

    # coach "В общем, тема обширная. Что именно ты хочешь узнать?" with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:393
translate english day1_ask_coach_woman_decline_marry_507f2311:

    # me "Что будет, если девушка откажется выходить замуж?" with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:397
translate english day1_ask_coach_woman_decline_marry_b83fcc9f:

    # coach "Если девушка живёт в селе, особо выбора-то и нет." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:401
translate english day1_ask_coach_woman_decline_marry_8adfbcdb:

    # coach "Она так же, как и односельчане, живёт по заведённым традициям и не представляет, что может быть иначе." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:405
translate english day1_ask_coach_woman_decline_marry_9f2fc59e:

    # coach "Если девушке прямо совсем неприятен мужчина, за которого она выходит замуж, то выбор невелик:" with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:409
translate english day1_ask_coach_woman_decline_marry_fba8ca90:

    # coach "она может сбежать из дома или совершить самоубийство. И то и другое сложно сделать, как ты понимаешь." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:413
translate english day1_ask_coach_woman_decline_marry_0ecff6e4:

    # coach "Если же дагестанка, допустим, училась в Махачкале, вкусила городской жизни, имеет широкий круг друзей и интересов," with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:417
translate english day1_ask_coach_woman_decline_marry_445f077d:

    # coach "получила диплом, собирается устроиться на работу и тут её вдруг родители хотят вернуть в село и выдать замуж за односельчанина - " with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:421
translate english day1_ask_coach_woman_decline_marry_8395dba2:

    # coach "вот тут уже не всё так однозначно." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:425
translate english day1_ask_coach_woman_decline_marry_33469ee5:

    # coach "Может быть, девушка сумеет отстоять свою независимость и переубедить родителей." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:429
translate english day1_ask_coach_woman_decline_marry_6c58cbe2:

    # coach "А может, договориться с родителями не удастся и тогда возможен любой исход, вплоть до убийства чести." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:433
translate english day1_ask_coach_woman_decline_marry_476eccfe:

    # me "Да уж." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:448
translate english day1_ask_coach_woman_love_c0d72114:

    # coach "По правилам парень и девушка не должны \"встречаться\", в том смысле как это происходит у русских." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:452
translate english day1_ask_coach_woman_love_d3963be6:

    # coach "Если парню понравилась девушка, он должен посвататься: просить руки и сердца у её родителей." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:456
translate english day1_ask_coach_woman_love_389417f0:

    # coach "Если родители дают одобрение, парень сможет взять девушку в жёны." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:460
translate english day1_ask_coach_woman_love_35272f98:

    # coach "Если вместо этого девушка будет ходить на свидания с парнем, она будет считаться распутной." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:464
translate english day1_ask_coach_woman_love_975a62f1:

    # coach "Разумеется, конкретные рамки допустимого могут варьироваться. В Махачкале студентка ДГУ может без проблем общаться со своими одногруппниками." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:468
translate english day1_ask_coach_woman_love_99b5c6fe:

    # coach "В то время как где-нибудь в селе девушка не осмелится даже заговорить с незнакомым мужчиной." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:472
translate english day1_ask_coach_woman_love_f92eccd8:

    # coach "Однозначно существует лишь запрет на ночевку в доме с чужим мужчиной. Всё остальное по ситуации." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:476
translate english day1_ask_coach_woman_love_07043fcf:

    # me "Интересно." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:491
translate english day1_ask_coach_woman_honor_killing_05e538bf:

    # coach "На Кавказе, конечно, это так не называют, там говорят \"смыть позор\"" with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:495
translate english day1_ask_coach_woman_honor_killing_1fa72983:

    # coach "Если девушка ведёт себя неподобающе, считается что она таким образом позорит весь свой род." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:499
translate english day1_ask_coach_woman_honor_killing_45f4dfdd:

    # coach "Кто-то из родственников-мужчин может в таком случае решить убить её, чтобы решить проблему." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:503
translate english day1_ask_coach_woman_honor_killing_28bac8f1:

    # coach "Обычно у родителей и братьев рука не поднимется убить свою дочь, сестру. Однако это может сделать кто-то из более дальних родственников." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:507
translate english day1_ask_coach_woman_honor_killing_63ef4e61:

    # coach "Чаще всего убивают за измену, блуд, отказ выходить замуж. Реже - за слухи." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:511
translate english day1_ask_coach_woman_honor_killing_05d4351f:

    # coach "Настоящие убийства чести — всё же редкое явление и служат скорее \"пугалом\"." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:515
translate english day1_ask_coach_woman_honor_killing_0c786f8c:

    # coach "Истории об убийствах чести циркулируют в обществе в виде тем, которые люди обсуждают или в виде слухов." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:519
translate english day1_ask_coach_woman_honor_killing_791ca23c:

    # coach "В реальной жизни обычно такие вопросы решаются без крови." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:523
translate english day1_ask_coach_woman_honor_killing_fe17819a:

    # me "Понятно." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:534
translate english day1_ask_coach_woman_kidnapping_9748720f:

    # me "Расскажи про традицию похищения невесты." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:538
translate english day1_ask_coach_woman_kidnapping_3880e775:

    # coach "Похищение невесты в Дагестане осуждается." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:542
translate english day1_ask_coach_woman_kidnapping_5aab9916:

    # coach "Если парню понравилась девушка, будет правильнее всего идти к её родителям просить руки и сердца." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:546
translate english day1_ask_coach_woman_kidnapping_9326b936:

    # coach "Похищение имеет смысл лишь в том случае, если не удаётся договориться нормально." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:550
translate english day1_ask_coach_woman_kidnapping_b11cfb7f:

    # coach "Это может быть когда парень и девушка любят друг друга, но родители против свадьбы." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:554
translate english day1_ask_coach_woman_kidnapping_c6ab7776:

    # coach "Второй вариант похуже: парень хочет взять девушку в жёны, но она против и родители её поддерживают." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:558
translate english day1_ask_coach_woman_kidnapping_b40b6086:

    # coach "В первом случае парень и девушка заранее договариваются о похищении и это происходит скорее для вида." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:562
translate english day1_ask_coach_woman_kidnapping_14be5c9a:

    # coach "Но может быть, что девушка реально не хочет выходить за парня, родители также против, но жених очень хочет жениться именно на ней." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:566
translate english day1_ask_coach_woman_kidnapping_814bbdbd:

    # coach "Тогда он действительно может организовать похищение, и довольно жестокое." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:570
translate english day1_ask_coach_woman_kidnapping_8e27c51e:

    # coach "Девушку завернут в бурку, кинут в автомобиль и отвезут в дом жениха." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:574
translate english day1_ask_coach_woman_kidnapping_e188d7ea:

    # coach "Дальше, если девушка провела ночь в доме мужчины, родителям приходится согласиться выдать дочь за него замуж под давлением общественного мнения." with dissolve
    coach "" with dissolve

# game/dagestan_exposition.rpy:578
translate english day1_ask_coach_woman_kidnapping_f130c1ae:

    # me "Ясно." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:588
translate english day1_ask_coach_knowall_f2c7b02b:

    # me "Да, я знаю о Дагестане и о традициях его жителей." with dissolve
    me "" with dissolve

# game/dagestan_exposition.rpy:597
translate english day1_ask_coach_learnedall_d80e2f8f:

    # me "Спасибо, я узнал всё, что мне нужно." with dissolve
    me "" with dissolve

translate english strings:

    # game/dagestan_exposition.rpy:12
    old "\"Почти ничего не знаю.\""
    new ""

    # game/dagestan_exposition.rpy:12
    old "\"Да, я знаю о Дагестане и о традициях его жителей.\""
    new ""

    # game/dagestan_exposition.rpy:120
    old "Религия в Дагестане."
    new ""

    # game/dagestan_exposition.rpy:120
    old "Традиции Дагестана."
    new ""

    # game/dagestan_exposition.rpy:120
    old "Положение женщины в обществе Дагестана."
    new ""

    # game/dagestan_exposition.rpy:120
    old "\"Спасибо, я узнал всё, что нужно.\""
    new ""

    # game/dagestan_exposition.rpy:367
    old "\"Что будет, если девушка откажется выходить замуж?\""
    new ""

    # game/dagestan_exposition.rpy:367
    old "Похищение невесты."
    new ""

    # game/dagestan_exposition.rpy:367
    old "Вернуться назад."
    new ""
